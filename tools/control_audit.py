...
def jobs_started(lines: list[str], index: int) -> bool:
    return any(line == "jobs:" for line in lines[: index + 1])
...
def action_ref_is_pinned(uses: str) -> bool:
    # Local/composite actions are not external third-party dependencies.
